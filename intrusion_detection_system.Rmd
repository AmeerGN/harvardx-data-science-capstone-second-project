...
bind_rows(lapply(X = binary_class_models, FUN = tuning_table)) %>%
  kbl(caption = "Binary Classification Tuning", col.names = c("Classifier", "Parameters", "Accuracy", "Tuning Time (seconds)")) %>%
  kable_styling(latex_options = c("HOLD_position"), position = "center") %>%
  column_spec(1, border_left = T) %>%
  column_spec(4, border_right = T) %>%
  row_spec(0, bold = T)
...
Figure \@ref(fig:binary-training-times-fig) shows the training times of the best tuned binary classifiers. 

```{r binary-training-times-fig, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Training Time of Best Tuned Binary Classifiers", fig.pos = "ht!", out.extra = ""}
...
binary_classifiers_times %>%
  ...
  scale_y_log10() +
  labs(title = "Training Time of Best Tuned Binary Classifiers", x = "Classifier", y = "Time (seconds)")
```

### Features Importance

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "T", fig.pos = "ht!", out.extra = ""}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="F", out.extra=""}
binary_metrics %>%
...
Sensitivity is same as detection rate, and recall
...
We applied the flow proposed in Section \@ref(methodology-classifiers) to the six filters mentioned in Table \@ref(tab:classifiers-desc-tab) with the default tuning grid values from `caret` on the multi-class classification IDS. In this section, we will analyze the training time of the parameters tuning and the final model, the optimal tuned parameters, and the features importance.

## Multi-class Classification Training Analysis

### Training Time

```{r multi-training-times-fig, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Training Time of Best Tuned Multi-class Classifiers", fig.pos = "ht!", out.extra = ""}
...
multi_classifiers_times %>%
  ...
  geom_label(aes(label = sprintf('%0.5f', final))) +
  scale_y_log10() +
  labs(title = "Training Time of Best Tuned Multi-class Classifiers", x = "Classifier", y = "Time (seconds)")
```
...
bind_rows(lapply(X = multi_class_models, FUN = tuning_table)) %>%
  kbl(caption = "Multi-class Classification Tuning", col.names = c("Classifier", "Parameters", "Accuracy", "Tuning Time (seconds)")) %>%
  kable_styling(latex_options = c("HOLD_position"), position = "center") %>%
  column_spec(1, border_left = T) %>%
  column_spec(4, border_right = T) %>%
  row_spec(0, bold = T)
